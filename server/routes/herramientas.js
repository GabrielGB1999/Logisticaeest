...
import { leerPlanilla, texto, numero, resolverCategoria, estadoValido, clavesDe, ALIAS } from '../planillas.js'
...
router.post('/import', requirePermiso('herramientas_editar'), async (req, res) => {
  ...
  try {
    const filas = leerPlanilla(HERRAMIENTAS_XLSX, clavesDe(ALIAS.codigo, ALIAS.nombre, ALIAS.descripcion, ALIAS.categoria, ALIAS.ubicacion, ALIAS.estado, ALIAS.stockActual, ALIAS.stockMinimo))
    await db.run('BEGIN')
